...
const minutesFromTime = (t: string): number => {
  if (t === 'All-day') return 24 * 60 + 1; // sentinel so timed events sort before all-day
...
const ExportPdfButton: React.FC<ExportPdfButtonProps> = ({ tripId, className }) => {
    ...
    return (
      hotelStays
        ?.filter(stay => {
          ...
          return d >= parseISO(stay.hotel_checkin_date) && d <= parseISO(stay.hotel_checkout_date);
        })
        .map(stay => {
          const d = parseISO(day.date);
          ...
          const rawTime = isCheckIn ? stay.checkin_time : isCheckOut ? stay.checkout_time : null;
    ...
    return (
      transportations
        ?.filter(t => {
          ...
          const d = parseISO(day.date);
          return fnsFormat(start, 'yyyy-MM-dd') === fnsFormat(d, 'yyyy-MM-dd');
        })
        .map(t => ({
          type: 'transportation',
          title:
  ...
  const getItemIcon = (type: string) =>
    type === 'accommodation' ? '🏨' : type === 'transportation' ? '✈️' : '🗓️';

  const shouldShowItemDetails = (item: any, options: PdfExportOptions) => ({
    showDetails:
      options.detailLevel === 'full' || (options.detailLevel === 'summary' && item.details),
  ...
  /* ------------------------------------------------------------------
    🔹 STYLE & HTML HELPERS
  ------------------------------------------------------------------- */
  const getStyles = (options: PdfExportOptions, tripHasHeroImg: boolean) => {
    const layoutClass =
      options.layout === 'timeline' ? 'timeline' : options.layout === 'daily' ? 'daily' : 'list';
    ...
    return {
      ...
      `,
      heroImg: tripHasHeroImg && options.showImages
        ? `background:url('${tripHasHeroImg}') center/cover;position:absolute;inset:0;z-index:0;`
        : '',
      heroText: `
        position:relative;z-index:1;color:${tripHasHeroImg && options.showImages ? '#fff' : '#37352f'};text-shadow:${tripHasHeroImg && options.showImages ? '0 1px 3px rgba(0,0,0,.3)' : 'none'};
      `,
  ...
  const generateDaySection = (
    ...
    const timelineRows = day.timelineItems.length
      ? day.timelineItems
          .map((item: any) => {
              ...
              <div class="day-row" style="${styles.dayRow}">
                ...
                <article style="${styles.card}">
                  <div style="display:flex;gap:16px;">
                    <div style="flex:1;">
                      <h3 style="margin:0 0 4px;font-size:16px;font-weight:600;">${icon} ${
              item.title
            }</h3>
                      ${
                        showDetails && item.details
                          ? `<div style="margin-bottom:4px;font-size:14px;">${item.details}</div>`
                          : ''
                      }
  ...
  const buildHtmlDocument = ({
    ...
    } Itinerary</title><style>${styles.page}</style></head><body>`;
    ...
    // timeline container
    html += `<div class="${options.layout}" style="${styles.timelineWrap}">`;

    processedDays.forEach(day => {
      html += generateDaySection(day, options, styles);
    });
  ...
  ------------------------------------------------------------------- */
  const createCustomPdf = useCallback(
    async (options: PdfExportOptions) => {
      ...
      try {
        ...
        /* 3 ▸ Styles -------------------------------------------------- */
        const styles = getStyles(options, Boolean(tripData.cover_image_url && options.showImages));
    ...
    [tripId, days, hotelStays, transportations],
  );
