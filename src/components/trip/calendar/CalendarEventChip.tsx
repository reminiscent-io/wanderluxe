...
import { CATEGORY_ICON_CLASS, type EventCategory } from '@/components/trip/day/components/timeline-utils';
...
const CalendarEventChip: React.FC<{ arg: EventContentArg }> = ({ arg }) => {
  const { entityType, category, tzBadge } = arg.event.extendedProps as { entityType: CalendarEntityType; category?: EventCategory; tzBadge?: string };
  const Icon = ICONS[entityType] ?? MapPin;
  // Full-strength category ink, as on the timeline; the block behind it carries the pastel.
  const iconTint = (category && CATEGORY_ICON_CLASS[category]) ?? '';
    ...
    return (
      <div className="wl-chip-stacked flex h-full min-w-0 flex-col gap-0.5 px-2 py-1" data-entity-type={entityType}>
        <span className="flex min-w-0 items-center gap-1.5">
          <Icon className={`wl-chip-icon h-3 w-3 shrink-0 ${iconTint}`} aria-hidden data-testid={`chip-icon-${entityType}`} />
          <span className="truncate font-sans text-xs font-medium leading-tight">{arg.event.title}</span>
        </span>
        {/* 70% ink clears 5.3:1 on every category pastel; 60% fell to ~4:1. */}
        {timeText && <span className="wl-chip-time truncate text-[10px] leading-tight tabular-nums opacity-70">{timeText}</span>}
      </div>
    );
  }
  return (
