-- copy_public_trip: carry the reservation end time into the copy.
--
-- The function enumerates reservation columns by hand, so a new column is
-- silently dropped until it is listed here — a copied showcase trip would
-- lose every dining end time with no error anywhere. Replacing the whole
-- function is the only option: editing the original migration in place does
-- nothing to an already-migrated database.
--
-- Identical to 20260819000001_copy_public_trip.sql apart from `end_time` in
-- the reservations INSERT/SELECT lists.
...
BEGIN
  ...
  INSERT INTO public.reservations (
    id, trip_id, day_id, restaurant_name, reservation_time, end_time, number_of_people,
  ...
  SELECT gen_random_uuid(), new_trip_id, dm.new_id, r.restaurant_name, r.reservation_time,
         r.end_time, r.number_of_people, r.address, r.phone_number, r.website, r.place_id,
         r.rating, r.notes, r.image_url, r.cost, r.currency, r.order_index, r.timezone
