...
OTHER ACCOMMODATIONS:${accommodations?.length > 1 ? accommodations.slice(1).map(acc => `
...
EXISTING RESTAURANT RESERVATIONS:${reservations?.length ? reservations.map(res => `
...
BUDGET / EXPENSES:${expenses?.length ? expenses.map(e => `
  - ${e.description}: ${e.cost} ${e.currency} (${e.expense_type})`).join("") : "\n  - No expenses tracked yet"}

USER INTERESTS / VISION BOARD:${visionBoard?.length ? visionBoard.map(v => `
...
`;
            ...
            messages: [
              ...
              },
              {
                ...
                content: [
                  {
                    type: "text",
                    text: "Please analyse this receipt and extract the travel data:"
                  },
                  {
                    type: "image_url",
                    image_url: {
    ...
    const aiBaseMessage = perplexJson.choices?.[0]?.message?.content ?? "Sorry, I could not generate a response. Please try again.";
    ...
    const [{ error: userInsertErr }, { error: aiInsertErr }] = await Promise.all([
